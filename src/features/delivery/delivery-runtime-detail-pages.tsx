...
type RuntimeKind = 'build' | 'workflow' | 'release' | 'release_bundle' | 'execution_task'
...
type RuntimeDetailResponse = ApiResponse<RuntimeObjectDetail<RuntimeRecord>>
...
function renderMetadata(value?: Record<string, unknown>) {
  ...
  return text ? <pre className="soha-system-json-block">{text}</pre> : <ManagementState bordered={false} compact title="暂无元数据" description="当前对象没有附加元数据。"/>
}

function runtimeDetailPath(kind: RuntimeKind, id: string) {
...
function buildRuntimeBindingSummary(
  ...
  const summary: DeliveryApplicationBindingSummary = {
    ...
    requiresApproval: Boolean(environment?.requiresApproval || binding.releasePolicy?.requiresApproval),
...
function buildApplicationDetailFromRuntime(
  ...
  const binding = buildRuntimeBindingSummary(kind, record, runtimeDetail.binding, runtimeDetail.environment, runtimeDetail)
...
function findMatchedBinding(detail?: DeliveryApplicationDetail, record?: RuntimeRecord, runtimeKind?: RuntimeKind) {
  ...
  if (directEnvironmentId) {
    const directMatch = bindings.find((binding) => binding.applicationEnvironmentId === directEnvironmentId)
  ...
  return bindings.find((binding) => {
    ...
    if (runtimeKind === 'workflow' && binding.workflowTemplate?.name && binding.latestWorkflow?.workflowName === (record as WorkflowRun)?.workflowName) return true
    return false
  })
}

function runtimeTemplateLabel(detail?: DeliveryApplicationDetail, record?: RuntimeRecord, runtimeKind?: RuntimeKind) {
...
function runtimeEnvironmentLabel(detail?: DeliveryApplicationDetail, record?: RuntimeRecord, runtimeKind?: RuntimeKind) {
  const binding = findMatchedBinding(detail, record, runtimeKind)
...
function runtimeEnvironment(detail?: DeliveryApplicationDetail, record?: RuntimeRecord, runtimeKind?: RuntimeKind): RuntimeEnvironment {
  const binding = findMatchedBinding(detail, record, runtimeKind)
  return binding
}
...
function runtimeSummaryItems(kind: RuntimeKind, record: RuntimeRecord | undefined, detail?: DeliveryApplicationDetail, runtimeDetail?: RuntimeObjectDetail<RuntimeRecord>) {
  ...
  return [
    ...
    { key: 'templateId', label: '模板 ID', children: binding?.workflowTemplateId || binding?.buildSourceId || '-' },
    { key: 'artifactCount', label: 'Artifacts', children: runtimeDetail?.artifacts?.length ?? String(runtimeDetail?.evidence?.artifactCount ?? '-') },
  ]
}

function buildTimelineItems(items: Array<{ id: unknown; title: unknown; status: unknown; summary?: unknown; createdAt?: unknown }>) {
  return items.map((item, index) => {
    ...
    const createdAt = item.createdAt === undefined || item.createdAt === null ? '' : String(item.createdAt)
    return {
      key: id,
      content: (
        <Space orientation="vertical" size={0}>
          <Text strong>{title}</Text>
          <Text type="secondary">{status}{summary ? ` · ${summary}` : ''}</Text>
...
function runtimeAssociationHints(kind: RuntimeKind, record: RuntimeRecord | undefined, links?: RuntimeObjectLinks, navigate?: (path: string) => void) {
  if (!record) return null
  const applicationPath = links?.application || (kind === 'build'
  ...
  return (
    <Space wrap size={[8, 8]}>
      <Button icon={<LinkOutlined />} onClick={() => navigate?.(applicationPath)}>打开应用运行态</Button>
      <Button icon={<LinkOutlined />} onClick={() => navigate?.(links?.audit || runtimeAuditPath(kind, record.id))}>审计日志</Button>
      <Button icon={<LinkOutlined />} onClick={() => navigate?.(links?.operations || runtimeOperationPath(kind, record.id))}>操作日志</Button>
      {links?.artifacts ? <Button icon={<LinkOutlined />} onClick={() => navigate?.(links.artifacts!)}>Artifacts</Button> : null}
    </Space>
  )
}

function RuntimeHeader({ kind, record, detail }: { kind: RuntimeKind, record?: RuntimeRecord, detail?: DeliveryApplicationDetail }) {
  return (
    <Space wrap size={[8, 8]}>
      <Tag color={runtimeStatus(record) === 'failed' ? 'red' : runtimeStatus(record) === 'running' ? 'blue' : 'green'}>{runtimeStatus(record)}</Tag>
...
function renderArtifactTimeline(artifacts?: ExecutionArtifact[], emptyDescription = '当前对象没有返回可展示的制品。') {
  return artifacts?.length ? (
    <Timeline items={buildTimelineItems(artifacts.map((artifact, index) => ({
      id: artifact.id || `${artifact.kind}:${artifact.name || artifact.ref || artifact.digest || index}`,
      title: `${artifact.kind}${artifact.name ? ` · ${artifact.name}` : ''}`,
      status: artifact.status || 'available',
      summary: artifact.ref || artifact.digest || artifact.path || artifact.workflowNodeId || '-',
...
function RuntimeEvidenceCard({ kind, record, navigate, links, bundleArtifacts, executionArtifacts, executionLogs }: {
  kind: RuntimeKind
  record?: RuntimeRecord
  ...
  if (kind === 'workflow') {
    ...
    const timelineItems = buildTimelineItems((workflow.nodeRuns?.length ? workflow.nodeRuns : workflow.steps).map((item, index) => ({
    ...
    return (
      <Card title="关键证据" className="soha-detail-card" size="small">
        ...
        <Paragraph className="soha-detail-card__paragraph" type="secondary">
          {workflow.clusterId || workflow.namespace || workflow.deploymentName ? `${workflow.clusterId || '-'} / ${workflow.namespace || '-'} / ${workflow.deploymentName || '-'}` : '没有额外运行证据。'}
        </Paragraph>
        <Text strong>制品</Text>
      ...
      <Card title="关键证据" className="soha-detail-card" size="small">
        <Space orientation="vertical" size={8}>
          ...
          {executionLogs?.length ? (
            <Timeline items={buildTimelineItems(executionLogs.map((item) => ({
              id: item.id,
              title: item.logLevel,
            ...
            <ManagementState bordered={false} compact title="暂无日志" description="该执行任务没有返回可展示的日志。"/>
          )}
          <Text strong>制品</Text>
          {renderArtifactTimeline(executionArtifacts, '该执行任务没有返回可展示的制品。')}
          <Text type="secondary">{task.releaseBundleId ? `releaseBundleId=${task.releaseBundleId}` : '没有关联版本包'}</Text>
        </Space>
        {runtimeAssociationHints(kind, record, links, navigate)}
      </Card>
    )
...
function RuntimeDetailPage({ kind }: { kind: RuntimeKind }) {
  const navigate = useNavigate()
  const params = useParams()
  const recordId = String(params[
    kind === 'build'
      ? 'buildId'
      : kind === 'workflow'
        ? 'workflowId'
  ...
  const runtimeDetailQuery = useQuery({
    queryKey: ['delivery-runtime-detail', kind, recordId],
    queryFn: () => api.get<RuntimeDetailResponse>(runtimeDetailPath(kind, recordId)),
    enabled: !!recordId,
  })

  if (!recordId) {
    return <div className="soha-page"><ManagementState kind="not-found" description={`${runtimeKindLabel(kind)} unknown 未找到`} /></div>
  }
  if (runtimeDetailQuery.isLoading) {
    return <div className="soha-page"><ManagementState kind="loading" /></div>
  }
  if (runtimeDetailQuery.isError && isNotFoundError(runtimeDetailQuery.error)) {
    return <div className="soha-page"><ManagementState kind="not-found" description={`${runtimeKindLabel(kind)} ${recordId || 'unknown'} 未找到`} /></div>
  }
  if (runtimeDetailQuery.isError) {
    return <div className="soha-page"><ManagementState kind="error" description={runtimeDetailQuery.error instanceof Error ? runtimeDetailQuery.error.message : '加载运行态详情失败'} /></div>
  }

  const runtimeDetail = runtimeDetailQuery.data?.data
  const record = runtimeDetail?.object
  if (!record) {
    return <div className="soha-page"><ManagementState kind="not-found" description={`${runtimeKindLabel(kind)} ${recordId || 'unknown'} 未找到`} /></div>
  }
  ...
  const executionArtifacts = kind === 'execution_task' || kind === 'workflow' ? runtimeArtifacts : undefined
  const executionLogs = Array.isArray(runtimeDetail?.evidence?.logs) ? runtimeDetail.evidence.logs as ExecutionLogItem[] : undefined
  ...
  return (
    <div className="soha-page">
      ...
      <Card className="soha-detail-card" title="基础信息" size="small">
        <Descriptions
          ...
          items={[
            ...
            { key: 'application', label: '关联应用', children: compactApplicationName(applicationDetail) },
            { key: 'environment', label: '关联环境', children: runtimeEnvironmentLabel(applicationDetail, record, kind) },
            { key: 'template', label: '关联模板/来源', children: runtimeTemplateLabel(applicationDetail, record, kind) },
            { key: 'createdAt', label: '创建时间', children: formatDateTime((record as BuildRecord | ReleaseRecord | ReleaseBundle | ExecutionTask | WorkflowRun).createdAt) },
            { key: 'updatedAt', label: '更新时间', children: formatDateTime((record as WorkflowRun | ReleaseBundle | ExecutionTask).updatedAt || (record as ReleaseRecord).deployedAt || (record as BuildRecord).finishedAt || (record as BuildRecord).startedAt || (record as ReleaseRecord).createdAt) },
          ]}
        />
      </Card>
      <Card className="soha-detail-card" title="关联信息" size="small">
        <Descriptions
          column={2}
          items={runtimeSummaryItems(kind, record, applicationDetail, runtimeDetail).map((item) => ({
            key: item.key,
            label: item.label,
        ...
        <Space wrap size={[8, 8]} style={{ marginTop: 16 }}>
          <Button icon={<LinkOutlined />} onClick={() => navigate(links?.application || runtimeApplicationPath(record.applicationId, kind === 'build' ? { buildId: record.id } : kind === 'workflow' ? { workflowRunId: record.id } : kind === 'release' ? { releaseId: record.id } : kind === 'release_bundle' ? { releaseBundleId: record.id } : { executionTaskId: record.id }))}>
            打开应用运行态
          </Button>
          <Button disabled={!permissions.canViewAudit} icon={<LinkOutlined />} onClick={() => navigate(links?.audit || runtimeAuditPath(kind, record.id))}>审计日志</Button>
          <Button disabled={!permissions.canViewOperations} icon={<LinkOutlined />} onClick={() => navigate(links?.operations || runtimeOperationPath(kind, record.id))}>操作日志</Button>
          <Button disabled={!permissions.canViewArtifacts || !links?.artifacts} icon={<LinkOutlined />} onClick={() => links?.artifacts && navigate(links.artifacts)}>Artifacts</Button>
        </Space>
      </Card>
      ...
      {kind === 'workflow' ? (
        <Card className="soha-detail-card" title="流程节点" size="small">
          <Timeline
            items={(record as WorkflowRun).nodeRuns?.map((node) => ({
              key: node.nodeId,
              content: (
                <Space orientation="vertical" size={0}>
                  <Text strong>{node.name}</Text>
                  <Text type="secondary">{node.status}{node.summary ? ` · ${node.summary}` : ''}</Text>
                </Space>
              ),
            })) ?? []}
          />
...
export function BuildDetailPage() {
  return <RuntimeDetailPage kind="build" />
}

export function WorkflowDetailPage() {
  return <RuntimeDetailPage kind="workflow" />
}

export function ReleaseDetailPage() {
  return <RuntimeDetailPage kind="release" />
}

export function ReleaseBundleDetailPage() {
  return <RuntimeDetailPage kind="release_bundle" />
}

export function ExecutionTaskDetailPage() {
  return <RuntimeDetailPage kind="execution_task" />
}
