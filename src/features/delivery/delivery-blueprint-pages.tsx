...
function createDefaultFileTemplate(index = 0) {
  return {
    ...
    required: index === 0,
    content: index === 0 ? 'FROM node:22-alpine\nWORKDIR /app\nCOPY . .\nRUN npm ci && npm run build\nCMD ["npm", "start"]\n' : '',
  }
}

function createBlueprintDraftValues(key = `app-template-${Date.now().toString(36)}`): BlueprintFormValues {
  return {
    key,
    name: '新建接入模板',
    description: '',
...
function buildBlueprintPayload(values: BlueprintFormValues, id?: string) {
  const buildSources = normalizeObjectArray(values.buildSources).map((source, index) => compactRecord({
  ...
  const environmentBindings = normalizeObjectArray(values.environmentBindings).map((binding, index) => compactRecord({
    ...
    resourceSelector: parseJSONObject(binding.resourceSelectorText, `环境绑定 ${index + 1} 资源选择器`),
    targets: parseJSONArray(binding.targetsText, `环境绑定 ${index + 1} 发布目标`),
  }))

  const files = normalizeObjectArray(values.files).map((file, index) => compactRecord({
...
function createListItemFromBlueprint(blueprint: DeliveryBlueprint, activeValues?: BlueprintFormValues): BlueprintListItem {
  if (activeValues) {
    return {
...
export function DeliveryBlueprintsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (values: Record<string, unknown>) => api.post<ApiResponse<DeliveryBlueprint>>('/delivery/blueprints', values),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, values }: { id: string; values: Record<string, unknown> }) => api.put<ApiResponse<DeliveryBlueprint>>(`/delivery/blueprints/${id}`, values),
    onSuccess: () => {
  ...
  const updateSearchParam = useCallback((blueprintId?: string) => {
  ...
  const applyFormValues = useCallback((values: BlueprintFormValues, dirtyAfterApply: boolean) => {
  ...
  const loadBlueprint = useCallback((blueprint: DeliveryBlueprint, options?: { dirtyAfterLoad?: boolean; formOverrides?: BlueprintFormValues; tabKey?: string }) => {
    const values = {
  ...
  useEffect(() => {
    ...
    const queryBlueprint = queryBlueprintId ? blueprints.find((item) => item.id === queryBlueprintId) : undefined
  ...
  const listItems = useMemo(() => {
    const items = blueprints.map((blueprint) => createListItemFromBlueprint(
      blueprint,
  ...
  const handleSave = async () => {
    try {
      ...
      const created = await createMutation.mutateAsync(payload) as ApiResponse<DeliveryBlueprint> | undefined
      const createdBlueprint = created?.data
      setFormSnapshot(values)
      setIsDirty(false)
  ...
  const handleBlueprintEnabledChange = (item: BlueprintListItem, enabled: boolean) => {
    if (item.id !== selectedBlueprintId) {
      if (!confirmDiscardChanges()) return
      if (item.blueprint) {
        loadBlueprint(item.blueprint, { dirtyAfterLoad: true, formOverrides: { enabled }, tabKey: 'basic' })
      }
      return
    }
  ...
  const designerTabs = [
    {
      ...
      children: (
        <div className="soha-delivery-blueprint-form-grid">
          ...
          <Form.Item className="soha-delivery-blueprint-switch-field" name="enabled" label="启用" valuePropName="checked">
            <Switch />
          </Form.Item>
          <Form.Item className="soha-delivery-blueprint-form-grid__wide" name="description" label="说明">
            <Input.TextArea rows={3} placeholder="适用语言、仓库形态、默认环境和平台规范说明" />
          </Form.Item>
          <Form.Item className="soha-delivery-blueprint-form-grid__wide" name="postCreateActions" label="接入后动作">
            <Select
              mode="tags"
              tokenSeparators={[',', '\n']}
              options={[
    ...
    },
    {
      ...
      children: (
        <div className="soha-delivery-blueprint-form-grid">
          ...
          <Form.Item name="appKey" label="应用 Key" rules={[{ required: true, message: '请输入应用 Key' }]}>
            <Input placeholder="sample-api" />
          </Form.Item>
          <Form.Item name="appGroup" label="应用分组" rules={[{ required: true, message: '请输入应用分组' }]}>
          ...
          <Form.Item className="soha-delivery-blueprint-form-grid__wide" name="repositoryPath" label="仓库路径">
          ...
          <Form.Item className="soha-delivery-blueprint-switch-field" name="appEnabled" label="应用启用" valuePropName="checked">
            <Switch />
          </Form.Item>
          <Form.Item className="soha-delivery-blueprint-form-grid__wide" name="appDescription" label="应用说明">
            <Input.TextArea rows={3} />
          </Form.Item>
          <Form.Item className="soha-delivery-blueprint-form-grid__wide" name="metadataText" label="应用元数据(JSON)">
            <Input.TextArea rows={5} spellCheck={false} placeholder='{"serviceKind":"kubernetes_workload"}' />
          </Form.Item>
        </div>
      ),
    },
          ...
          {(fields, { add, remove }) => (
            <div className="soha-delivery-blueprint-repeat-list">
              <div className="soha-delivery-blueprint-repeat-list__toolbar">
                ...
                <Button icon={<PlusOutlined />} onClick={() => add(createDefaultBuildSource(fields.length))}>添加构建源</Button>
              </div>
              {fields.length === 0 ? (
                <ManagementState bordered={false} compact kind="empty" title="暂无构建源" description="至少添加一个构建源，才能形成可发布的应用接入规范。" />
              ) : null}
              {fields.map((field, index) => (
                <div className="soha-delivery-blueprint-repeat-item" key={field.key}>
                  ...
                  <div className="soha-delivery-blueprint-form-grid">
                    <Form.Item name={[field.name, 'id']} label="Source ID" rules={[{ required: true, message: '请输入 Source ID' }]}>
                    ...
                    <Form.Item className="soha-delivery-blueprint-switch-field" name={[field.name, 'enabled']} label="启用" valuePropName="checked">
                      <Switch />
                    </Form.Item>
                    <Form.Item className="soha-delivery-blueprint-switch-field" name={[field.name, 'isDefault']} label="默认" valuePropName="checked">
                      <Switch />
                    </Form.Item>
                    <Form.Item className="soha-delivery-blueprint-form-grid__wide" name={[field.name, 'configText']} label="构建配置(JSON)">
                      <Input.TextArea rows={5} spellCheck={false} placeholder='{"contextDir":".","dockerfilePath":"Dockerfile"}' />
                    </Form.Item>
                  </div>
                </div>
              ))}
      ...
      children: (
        <Form.List name="environmentBindings">
          {(fields, { add, remove }) => (
            <div className="soha-delivery-blueprint-repeat-list">
              <div className="soha-delivery-blueprint-repeat-list__toolbar">
                <Text type="secondary">定义接入后默认环境、发布流程、构建策略和目标资源选择方式。</Text>
                <Button icon={<PlusOutlined />} onClick={() => add(createDefaultEnvironmentBinding(fields.length))}>添加环境绑定</Button>
              </div>
              {fields.length === 0 ? (
                <ManagementState bordered={false} compact kind="empty" title="暂无环境绑定" description="可以先保存应用档案模板，后续再补环境绑定。" />
              ) : null}
              {fields.map((field, index) => (
                <div className="soha-delivery-blueprint-repeat-item" key={field.key}>
                  <div className="soha-delivery-blueprint-repeat-item__head">
                  ...
                  <div className="soha-delivery-blueprint-form-grid">
                    ...
                    <Form.Item className="soha-delivery-blueprint-switch-field" name={[field.name, 'requiresApproval']} label="需要审批" valuePropName="checked">
                      <Switch />
                    </Form.Item>
                    <Form.Item name={[field.name, 'approverRoles']} label="审批角色">
                      <Select mode="tags" tokenSeparators={[',', '\n']} placeholder="release-manager, ops-lead" />
                    </Form.Item>
                    <Form.Item className="soha-delivery-blueprint-switch-field" name={[field.name, 'autoRollback']} label="自动回滚" valuePropName="checked">
                      <Switch />
                    </Form.Item>
                    ...
                    <Form.Item className="soha-delivery-blueprint-form-grid__wide" name={[field.name, 'buildVariablesText']} label="构建变量(JSON)">
                      <Input.TextArea rows={4} spellCheck={false} />
                    </Form.Item>
                    <Form.Item className="soha-delivery-blueprint-form-grid__wide" name={[field.name, 'buildArgsText']} label="构建参数(JSON)">
                      <Input.TextArea rows={4} spellCheck={false} />
                    </Form.Item>
                    <Form.Item className="soha-delivery-blueprint-form-grid__wide" name={[field.name, 'resourceSelectorText']} label="资源选择器(JSON)">
                      <Input.TextArea rows={4} spellCheck={false} placeholder='{"matchLabels":{"app":"sample"}}' />
                    </Form.Item>
                    <Form.Item className="soha-delivery-blueprint-form-grid__wide" name={[field.name, 'targetsText']} label="发布目标(JSON Array)">
                      <Input.TextArea rows={5} spellCheck={false} placeholder='[{"clusterId":"dev","namespace":"default","workloadKind":"Deployment","workloadName":"sample"}]' />
                    </Form.Item>
                  </div>
                </div>
              ))}
          ...
          {(fields, { add, remove }) => (
            <div className="soha-delivery-blueprint-repeat-list">
              <div className="soha-delivery-blueprint-repeat-list__toolbar">
                ...
                <Button icon={<PlusOutlined />} onClick={() => add(createDefaultFileTemplate(fields.length))}>添加文件模板</Button>
              </div>
              {fields.length === 0 ? (
                <ManagementState bordered={false} compact kind="empty" title="暂无规范文件" description="模板可以只创建平台对象，也可以附带规范文件草稿。" />
              ) : null}
              {fields.map((field, index) => (
                <div className="soha-delivery-blueprint-repeat-item" key={field.key}>
                  <div className="soha-delivery-blueprint-repeat-item__head">
                  ...
                  <div className="soha-delivery-blueprint-form-grid">
                    <Form.Item name={[field.name, 'path']} label="路径" rules={[{ required: true, message: '请输入文件路径' }]}>
                    ...
                    <Form.Item className="soha-delivery-blueprint-switch-field" name={[field.name, 'required']} label="必需" valuePropName="checked">
                      <Switch />
                    </Form.Item>
                    <Form.Item className="soha-delivery-blueprint-form-grid__wide" name={[field.name, 'content']} label="内容">
      ...
      children: (
        <div className="soha-delivery-blueprint-advanced">
          <Form.Item name="executionHintsText" label="执行提示(JSON)">
            <Input.TextArea rows={5} spellCheck={false} placeholder='{"aiGatewaySkill":"delivery-developer"}' />
          </Form.Item>
    ...
    <div className="soha-page soha-delivery-blueprint-page">
      <div className="soha-delivery-blueprint-toolbar">
        <Space wrap>
          ...
          <Button icon={<EyeOutlined />} disabled={!hasSelection} loading={renderMutation.isPending} onClick={handleRenderSpec}>
            渲染规范
          </Button>
          <Button icon={<PlayCircleOutlined />} disabled={!hasSelection} loading={bootstrapMutation.isPending} onClick={handleBootstrap}>
            平台接入
          </Button>
        </Space>
        <Space wrap>
          {isDirty ? <Tag color="gold">未保存</Tag> : <Tag>已同步</Tag>}
          <Button icon={<ReloadOutlined />} loading={blueprintsQuery.isFetching} onClick={() => { if (confirmDiscardChanges()) void blueprintsQuery.refetch() }}>
            刷新
          </Button>
        </Space>
      </div>
