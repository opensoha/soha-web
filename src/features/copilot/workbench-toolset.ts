...
export const DEFAULT_RECOMMENDED_ADAPTER_IDS = ['platform-native.v1', 'logs.v1', 'metrics.v1', 'traces.v1']

export const TOOLSET_BUDGET_FIELDS = [
  { key: 'timeoutSeconds', label: 'Timeout Seconds', suffix: 's', description: '单次分析工具运行超时。' },
  { key: 'maxEvidenceItems', label: 'Max Evidence Items', suffix: 'items', description: '进入分析工件的证据数量上限。' },
  { key: 'maxQueries', label: 'Max Queries', suffix: 'queries', description: '预留给后续多工具编排的查询次数预算。' },
  { key: 'maxLogBytes', label: 'Max Log Bytes', suffix: 'bytes', description: '预留给日志工具的读取字节预算。' },
] as const
...
export function scopeOverrideState(value?: Record<string, unknown>) {
  ...
  if (Number.isFinite(timeRangeMinutes) && timeRangeMinutes > 0) out.timeRangeMinutes = timeRangeMinutes
  return out
}
...
export function canonicalDisabledToolNames(values: string[], adapters: WorkbenchAdapter[]) {
  return pruneEmptyStringList(values).map((value) => {
    ...
    const matches = adapters.filter((adapter) => (adapter.tools ?? []).some((tool) => tool.name === value))
...
export function buildDisabledToolOptions(adapters: WorkbenchAdapter[]) {
  return adapters.flatMap((adapter) => (adapter.tools ?? []).map((tool) => ({
    value: qualifiedToolName(adapter.id, tool.name),
    label: `${adapter.name} / ${tool.name}`,
  })))
}

export function recommendedAdapterIds(adapters: WorkbenchAdapter[], dataSources: WorkbenchDataSource[]) {
  const registered = new Set(adapters.map((item) => item.id))
  const enabledAdapterSet = new Set(dataSources
    .filter((item) => item.enabled && item.mcpAdapter)
  ...
  return pruneEmptyStringList([
    ...platformNative,
    ...fromDataSources,
    ...DEFAULT_RECOMMENDED_ADAPTER_IDS.filter((item) => item !== 'platform-native.v1' && registered.has(item) && enabledAdapterSet.has(item)),
  ])
}
