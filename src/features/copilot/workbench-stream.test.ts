...
describe('workbench stream helpers', () => {
  ...
  it('accumulates tool.delta output and logs on placeholders or existing tools', () => {
    ...
    const state = [initialDelta, started, nextDelta].reduce(reduceWorkbenchStreamState, createWorkbenchStreamState())
  ...
  it('canonicalizes historical agent.status values', () => {
    ...
    expect(reduceWorkbenchStreamState(createWorkbenchStreamState(), completed)).toEqual(expect.objectContaining({
      done: true,
      agentStatus: expect.objectContaining({ status: 'succeeded' }),
    }))
    expect(reduceWorkbenchStreamState(createWorkbenchStreamState(), canceled)).toEqual(expect.objectContaining({
      done: true,
      agentStatus: expect.objectContaining({ status: 'cancelled' }),
    }))
    expect(reduceWorkbenchStreamState(createWorkbenchStreamState(), timeout)).toEqual(expect.objectContaining({
      done: true,
