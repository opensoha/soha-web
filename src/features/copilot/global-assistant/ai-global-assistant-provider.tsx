...
import type { WorkbenchGlobalAssistantOpenRequest, WorkbenchSendMessageStreamRequest } from '@opensoha/contracts/gen/ts/sohaapi'
import { hasPermission } from '@/features/auth/permission-snapshot'
...
import { AIFloatingAssistantPanel } from './ai-floating-assistant-panel'
...
function sourceWorkbenchFromPath(pathname: string): AIPageContext['sourceWorkbench'] {
  ...
  if (pathname.startsWith('/virtualization') || pathname.startsWith('/virtualization-workbench')) return 'virtualization'
  return 'ai'
}

function recordGlobalAssistantEvent(action: WorkbenchGlobalAssistantOpenRequest['action'], context: AIPageContext, sessionId?: string) {
...
function routeContext(pathname: string, search: string): AIPageContext {
  return {
    ...
    sourceTitle: pathname === '/' ? 'Soha Overview' : pathname.split('/').filter(Boolean).join(' / '),
  }
}
...
function selectionSnapshotFromWindow(): { context: AISelectionContext; toolbar: AISelectionToolbarState } | null {
  ...
  const left = Math.min(Math.max((activeRect?.left ?? 16) + (activeRect?.width ?? 0) / 2, 16), window.innerWidth - 16)
  ...
  return {
    context: {
      ...
      sourceElementLabel: anchorElement?.getAttribute('aria-label') ?? anchorElement?.textContent?.trim().slice(0, 80),
    },
    toolbar: {
      left,
      top,
...
export function GlobalAIAssistantProvider({ children, permissionSnapshot }: GlobalAIAssistantProviderProps) {
  const location = useLocation()
  const navigate = useNavigate()
  const queryClient = useQueryClient()
  const { message } = AntdApp.useApp()
  const canUseChat = hasPermission(permissionSnapshot, 'observe.ai.chat')
  const routeFallbackContext = useMemo(() => routeContext(location.pathname, location.search), [location.pathname, location.search])
  ...
  const [streamState, setStreamState] = useState<WorkbenchStreamState>(() => createWorkbenchStreamState())
  ...
  const ensureSession = useCallback(async (context: AIPageContext, action: AIGlobalAssistantAction) => {
  ...
  const sendPrompt = useCallback(async (
    ...
    try {
      ...
      await streamWorkbenchMessage(
        ...
        (event) => {
          ...
          setMessages((items) => items.map((item) => {
            ...
            return {
              ...item,
              id: nextStreamState.message.done && nextStreamState.message.id ? nextStreamState.message.id : item.id,
      ...
      await queryClient.invalidateQueries({ queryKey: ['copilot-workbench-messages', session.id] })
      await queryClient.invalidateQueries({ queryKey: ['copilot-workbench-session-detail', session.id] })
      await queryClient.invalidateQueries({ queryKey: ['copilot-workbench-sessions'] })
      ...
      setMessages((items) => items.map((item) => (
        item.id === assistantMessageId
          ? { ...item, content: isAbort ? '已取消本次回复。' : error instanceof Error ? error.message : 'AI 请求失败。', status: isAbort ? 'abort' : 'error' }
          : item
      )))
      if (!isAbort) {
  ...
  const launchAssistant = useCallback(async (request: AIGlobalAssistantLaunchRequest) => {
    const context = mergeAIPageContext(activeContext, request.contextOverride)
    const selection = request.selection ?? (request.action.includes('selection') ? selectionContext ?? undefined : undefined)
  ...
  const openWorkbench = useCallback(() => {
    ...
    ;(['service', 'pod', 'node', 'entityKind', 'entityName', 'sourceWorkbench'] as const).forEach((key) => {
  ...
  useEffect(() => {
    ...
    const onContextMenu = (event: MouseEvent) => {
      ...
      const snapshot = selectionSnapshotFromWindow()
      const contextElement = event.target instanceof Element ? event.target.closest('[data-ai-context]') : null
      if (snapshot) {
